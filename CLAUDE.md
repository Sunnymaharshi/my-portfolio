...
|---|---|---|---|
| Orion | [72, 30, 18] | `#c8deff` | 7 (Betelgeuse, belt trio, Rigel…) — moved right + into the entry view, scale 7 |
...
**ButterflyNebula** (`ButterflyNebula.jsx`) — procedural **bipolar / butterfly nebula** (NGC-6302-style): two **wide, hollow, frilly flaring wings** (trumpet flare, fan-flattened in Z, swept outward) sharing a bright nucleus, plus a thin equatorial **dust ring** at the waist. Cool ramp (blue-white waist → blue → violet rims), brighter+clumpy along the rims so it reads as gas, not a cone. Additive points, `fog=false`. Stays **face-on** (a gentle scale "breathe", **no axial spin** — spinning turned it edge-on/line-like). `buildButterfly({count,length,flare})`, count scaled on mobile. Placed at `[150, 20, -105]`, `rotation=[0.2, π/2, 0.12]` (the π/2 turns the broad wings across the view). Kept off the galaxies' sky directions so they don't read as one blob.

**StarCluster** (`StarCluster.jsx`) — **globular cluster**: a dense, centrally-concentrated ball of cool blue-white stars (`r = radius·random²`) + a soft core glow. Additive points, `fog=false`, slow rotation, mobile-scaled count. Placed at `[-55, 50, 35]` (upper-left of the initial view; kept off the Cassiopeia line-of-sight so the fuzzy ball doesn't sit on the constellation).

**Galaxy** (`Galaxy.jsx`) — **4** distant **spiral-galaxy particle disks** as deep-background scenery, far off the flight path. Logarithmic-spiral arm distribution (`buildGalaxy`: radius^1.8 central concentration, per-arm branch angle + spin, radius-scaled randomness). Warm core → blue arms color lerp with rare pink young-star highlights; additive blending; a faint glowing core sphere. Slow per-galaxy rotation. Positions `[-220,120,-120]` (3-arm, scale 1) and `[260,-150,-260]` (2-arm, scale 0.7).
...
- **Hero (section 0)**: centered overlay — name, role, tagline, CTAs (View Projects / About / Contact → guided fly-to), controls hint. `pointer-events: none` except the CTA buttons, so it never blocks exploration. **Visibility is owned by `App` (`showHero`)**: shown at first load, hidden the moment the user explores (a window-level `wheel`/drag-`pointermove`/`touchmove` listener sets it false), and shown again **only** when the user taps **Home** in the NavMenu (`onHome` → `setShowHero(true)`). No idle auto-return (the old `useExploring` idle-fade was removed because the hero kept reappearing mid-exploration). **Hint text is device-aware**: mobile shows "Drag to look · pinch to fly", desktop shows "Drag to look · scroll to fly" (detected via `IS_MOBILE = matchMedia('(pointer: coarse)') || innerWidth < 820`).
...
- **Sections 1–4 (mobile ≤720px)**: the card becomes a **collapsible bottom sheet** so the 3D object stays visible. **Expand/collapse** via tapping the header OR **swiping up/down** anywhere on the sheet (threshold 40px, handled in `PanelShell` via `onTouchStart`/`onTouchEnd`). A **grab-handle pill** (`.grabHandle`) at the top of the sheet hints at swipeability. Collapsed (default) shows eyebrow + title + a soft **rounded SVG chevron** (replaces the old sharp `⌃` character); expanded reveals the scrollable `sheetBody`. Desktop ignores `expanded` (chevron hidden, body always shown). Scrim hidden on mobile; the NavMenu collapses to a single compact row (Résumé/Tour dropped — Résumé still in the Contact sheet). Sheet heights use **`dvh`** (not `vh`) so the mobile browser's dynamic URL bar doesn't clip the top. (`EdgeHints` markers render on mobile too.)
- Content shapes per `kind`: `about` (stats grid + experience + certs + education), `skills` (category groups → chips), `projects` (cards: name, tagline, blurb, bullets, stack chips, "View code" link), `contact` (link rows + Résumé button). **Contact on mobile**: links use a 2-row CSS grid layout (label above value, arrow/copy icon spanning both rows) so long email addresses never wrap mid-line. **Email row has a clipboard copy button** (SVG icon, `navigator.clipboard.writeText`) — clicking it copies the address without opening the mail app; shows a checkmark for 2 s.

**LoadingScreen** — fixed fullscreen overlay (`z-index: 100`). **Background is `#050d1a`** (`--bg-deep`), matching the canvas CSS background and the WebGL clear color (`gl.setClearColor('#050d1a', 1)` in `onCreated`) — so all three layers are identical during the dissolve and the transition is seamless. **Constellation name reveal**: a dense full-screen starfield (DOM `div` dots, ~320), with small star-nodes arranged into the letters of **MAHARSHI / REDDY**; a thin luminous `<polyline>` traces **each letter** (letters are NOT joined). Single-stroke glyphs in `GLYPHS` (unit-grid point arrays); `layoutLine` produces one polyline per letter; the global letter order drives a draw stagger. The name-nodes are deliberately small so that when the line fades they dissolve into the starfield rather than spelling the name in bright dots.
...
15. **Favicon** — `public/favicon.svg`. Transparent background (no rect) — no white border on any browser theme. Planet with atmosphere glow + split ring (back half dimmer, front half brighter) + sparkle, all in the moonlit palette. `index.html` references it as `type="image/svg+xml"`.
...
6. ~~**Mobile bottom-sheet polish** `[mobile]` — add swipe-up-to-expand (not just tap) + a grab-handle visual~~ ✓ Done — swipe gestures + grab handle added; hero hint is now device-aware (pinch vs scroll).
...
**Smaller niceties:**
8. ~~**Copy-email button** on Contact `[both]` — tap-to-copy beside the `mailto:`~~ ✓ Done — clipboard SVG icon on the email row; checkmark feedback for 2 s.
