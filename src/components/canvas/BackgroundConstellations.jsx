...
const CONSTELLATIONS = [
  {
    ...
    lines: [[0, 2], [1, 4], [2, 3], [3, 4], [5, 2], [6, 4], [0, 1]],
  },
  {
    id: 'big-dipper',
    center: [148, 78, 15],
  ...
  },
  {
    ...
    lines: [[0, 1], [1, 2], [2, 3], [3, 4], [4, 0], [0, 5], [5, 6], [6, 1]],
  },
]
...
function ConstellationShape({ data }) {
  ...
  const lineBuffers = useMemo(() =>
    data.lines.map(([a, b]) => new Float32Array([...positions[a], ...positions[b]]))
  , [positions])
  ...
  useFrame((state) => {
    ...
    starRefs.current.forEach((mesh, i) => {
      ...
      const s = 0.55 + 0.45 *
        Math.sin(t * 0.9 + i * 1.37) *
        Math.sin(t * 1.4 + i * 0.83)
    ...
    <group>
      ...
      {lineBuffers.map((buf, i) => (
        <line key={i}>
          ...
          <lineBasicMaterial color={data.color} transparent opacity={0.16} fog={false} />
        </line>
      ))}
    </group>
  )
...
export default function BackgroundConstellations() {
  return (
    <>
      {CONSTELLATIONS.map(c => <ConstellationShape key={c.id} data={c} />)}
    </>
  )
}
